...
public class CategoryPublicController {
    ...
    public CategoryDto getById(@NotNull @PathVariable(name = "catId") Long id) {
        ...
        CategoryDto getCategoryDtoById = categoryService.getCategoryById(id);

        return ResponseEntity.ok().body(getCategoryDtoById).getBody();
    }
    ...
    public List<CategoryDto> getAll(@PositiveOrZero @RequestParam(defaultValue = "0") Integer from,
        ...
        List<CategoryDto> getAllCategoriesDto = categoryService.getAllCategories(pageRequest);

        return ResponseEntity.ok().body(getAllCategoriesDto).getBody();
    }
}