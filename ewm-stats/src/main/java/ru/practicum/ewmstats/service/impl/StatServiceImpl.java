...
import static ru.practicum.ewmstats.mapper.EndpointHitMapper.*;
...
public class StatServiceImpl implements StatService {
    ...
    public EndpointHitDto save(EndpointHitDto endpointHitDto) {
        EndpointHit endpointHit = dtotoEndpointHit(endpointHitDto);
        endpointHit.setTimestamp(LocalDateTime.now());
        log.info("Сохранена информация о том, что к эндпоинту был запрос.");

        return endpointHitToDto(statRepository.save(endpointHit));
    }
